-- Migration: Fix accept_invitation_with_sync - Complete rewrite
-- Date: 2026-03-30
--
-- This migration fixes multiple bugs in the accept_invitation_with_sync RPC:
--
-- BUG 1 (Critical): The most recent version (add_linked_profile_to_invitation.sql)
--   removed steps 9 & 10 which copy existing profile_details from source profiles
--   to synced copies. This means on acceptance, the synced profiles have the correct
--   name/birthday/email but NO clothing, medical, gift, hobby, or other detail data.
--   New items added AFTER acceptance sync correctly via triggers, but pre-existing
--   data is missing.
--
-- BUG 2: The sharing_categories array was missing 'important_accounts', so that
--   category's sharing preference was never saved during acceptance.
--
-- BUG 3: The sharing preferences INSERT used ON CONFLICT (profile_id, category)
--   which is a 2-column constraint. The actual unique constraint is
--   (profile_id, target_user_id, category). Without target_user_id, the preferences
--   are not per-connection and conflict resolution fails or overwrites wrong rows.
--
-- BUG 4: The propagation trigger was not disabled during bulk detail copy, causing
--   each INSERT to fire the trigger and potentially create duplicate entries or
--   cascade incorrectly.
--
-- NEW: Acceptor sharing preferences are now also created during acceptance. The RPC
--   accepts p_acceptor_sharing_prefs (JSONB) to control what the acceptor shares back.
--   If not provided, all categories default to shared (except important_accounts).
--
-- This version merges the best of:
--   - add_linked_profile_to_invitation.sql (linked_profile_id resolution, acceptor account resolution)
--   - fix_sharing_preference_direction.sql (detail copy with trigger disable, correct sharing prefs)
