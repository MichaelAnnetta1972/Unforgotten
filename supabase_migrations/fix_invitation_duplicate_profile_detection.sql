-- Migration: Fix duplicate profile detection in accept_invitation_with_sync
-- Problem: The iOS client cannot query the inviter's profiles before joining
-- (no RLS access yet), so the client-side duplicate check is unreliable.
-- Fix: The RPC function (SECURITY DEFINER) now auto-detects an existing profile
-- in the inviter's account by matching the invitation email, and links to it
-- instead of creating a new one — even when p_existing_profile_id is not passed.
...
BEGIN
    ...
    -- 5. Resolve existing profile: use p_existing_profile_id if provided, otherwise
    -- auto-detect by matching the invitation email in the inviter's account.
    -- This is the server-side fallback for when RLS prevented the client from
    -- querying the inviter's profiles before joining.
    ...
    IF v_resolved_existing_profile_id IS NULL AND v_invitation.email IS NOT NULL AND v_invitation.email != '' THEN
        ...
        WHERE account_id = v_invitation.account_id
          ...
          AND sync_connection_id IS NULL  -- only match unlinked profiles
        LIMIT 1;
    ...
    -- 8. Create synced profile of inviter in acceptor's account (if acceptor has an account)
    IF p_acceptor_account_id IS NOT NULL THEN
    ...
    -- 9. Create sharing preference records based on invitation settings
