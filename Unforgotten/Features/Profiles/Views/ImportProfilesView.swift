    ...
    var body: some View {
        NavigationStack {
            ZStack {
                ...
                ScrollView {
                    VStack(spacing: 16) {
                        Text("Tap a row to include or exclude it, then tap Import.")
                            .font(.appCaption)
                        ...
                        ForEach($rows) { $row in
                            ProfileRowCard(row: $row, accentColor: appAccentColor)
                                .padding(.horizontal, AppDimensions.screenPadding)
                        }
    ...
    private func save() async {
        ...
        for row in rows where row.isSelected {
            ...
            do {
                ...
                #if DEBUG
                print("Failed to import profile \(trimmed): \(error)")
                #endif
            }
        }
...
private struct ProfileRowCard: View {
    @Binding var row: ImportProfilesView.Row
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if row.isSelected,
               (row.phone?.isEmpty == false) || (row.email?.isEmpty == false) || (row.address?.isEmpty == false) {
                Divider()
